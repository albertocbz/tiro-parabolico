...
st.write(
    "Ajusta los parámetros con los controles de la izquierda y haz clic en **Disparar**."
)
st.sidebar.header("🎛️ Parámetros del Cañón")
...
try:
  ...
  st.error(
      f"Falta una de las imágenes necesarias en la carpeta: {e}. Asegúrate de"
      " subir todas."
  )
  st.stop()
...
if "disparado" not in st.session_state:
  st.session_state.disparado = False
if reiniciar:
  st.session_state.disparado = False
  st.rerun()
if disparar:
  st.session_state.disparado = True
...
ancho_rotado, alto_rotado = canon_rotado.size
ancho = ancho_rotado / escala
alto = alto_rotado / escala
axis.imshow(
    canon_rotado,
